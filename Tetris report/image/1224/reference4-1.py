...
def Simulate_Punishing(InputTetris, Allblocksign, SimulateColumnSign):
    emptyarea = surrounding(InputTetris)
    #print(emptyarea)
    for i in range(len(emptyarea)):
        if Allblocksign[emptyarea[i][1]+AboveLine][emptyarea[i][0]] == '1' :
            Allblocksign[emptyarea[i][1]+AboveLine][emptyarea[i][0]] = '0'
        if SimulateColumnSign[emptyarea[i][0]][emptyarea[i][1]+AboveLine] == '1':
            SimulateColumnSign[emptyarea[i][0]][emptyarea[i][1]+AboveLine] = '0'
...
def computer(InputTetris, Allblocks):
    ...
    original = SimulateAllblocksign
    SimulateColumnSign = AllColumnSign(SimulateAllblocksign)
    original_ = SimulateColumnSign
    
    board_holes_record = BoardBuriedHoles(original_)
    board_wells_record = BoardWells(original_)
    ...
    while count <= SimulateTetris.rotationtype() - 1:
        ...
        for j in range(_leftmost, _rightmost):
            ...
            SimulateTetris.center[1] = SimulateTetris.center[1] - topmost(SimulateTetris)
            move = j - keep_x
            spare.append(move) # to test the number of steps 
            SimulateAllblocks = Allblocks
            SimulateAllblocksign = original
            SimulateColumnSign = original_
            #print(SimulateColumnSign)
            a_1_1 = time.time()
            #print('a_1_1 - a_1', a_1_1 - a_1)
            ...
            for location in range(len(check)):
                ...
                SimulateColumnSign[columnlocation][rowlocation+AboveLine] = '1'
            a_1_3 = time.time()
            #print('a_1_3 - a_1_2', a_1_3 - a_1_2)   
            #SimulateColumnSign = AllColumnSign(SimulateAllblocksign) 
            ...
            if SimulateTetris.color == WHITE:
                Simulate_Punishing(SimulateTetris, SimulateAllblocksign, SimulateColumnSign)    
            a_2 = time.time()
            #print('a_2 - a_1_3', a_2 - a_1_3)
            #SimulateColumnSign = AllColumnSign(SimulateAllblocksign)
            #print(SimulateColumnSign)
            a_2_1 = time.time()
            #print('a_2_1 - a_2', a_2_1 - a_2)
            value = evaluate(SimulateAllblocksign, SimulateColumnSign, SimulateTetris, board_holes_record, board_wells_record)
            print(value)
            a_2_2 = time.time()
            #print('a_2_2 - a_2_1', a_2_2 - a_2_1)
            _trials.append(value)
            a_3 = time.time()
            #print('a_3 - a_2_2', a_3 - a_2_2)
            ...
            for location in range(len(check)):
                ...
                SimulateColumnSign[columnlocation][rowlocation+AboveLine] = '0'
            #SimulateColumnSign = AllColumnSign(SimulateAllblocksign) 
...
def LandingHeight(InputTetris, column):
    locate = bottom(InputTetris)
    
    return 19 - locate + 0.5 * len(isleft(InputTetris))
def ErodedPieceCellsMetric(InputTetris, Allblocksign):
    ...
    record_in_a_row = []
    for i in range(len(check)):
        if check[i][1] not in record:
            record[check[i][1]] = [check[0]]
        else:
            record[check[i][1]].append(check[0])
        record_in_a_row.append(check[i][1])
        for j in range(len(Allblocksign[check[i][1]])):
            if Allblocksign[check[i][1]][j] == '0':
                record_in_a_row.pop(-1)
                break
    for i in range(len(record_in_a_row)):
        count += len(record[record_in_a_row[i]]) 
    return len(record_in_a_row) * count
def RowTransitions(InputTetris, Allblocksign):
    check = InputTetris.position()
    setup = {}
    record = []
    result = 0
    for i in range(len(check)):
        if check[i][1] not in setup:
            setup[check[i][1]] = [check[i][0]]
        else:
            setup[check[i][1]].append(check[i][0])
    for i in range(len(check)):
        if min(setup[check[i][1]])- 1 > 0:
            if Allblocksign[check[i][1]][min(setup[check[i][1]]) - 1] == '1':
                record.append(1)
            else:
                record.append(0)
        else:
            record.append(1)
        if max(setup[check[i][1]]) + 1 < 9:
            if Allblocksign[check[i][1]][max(setup[check[i][1]]) + 1] == '1':
                record.append(1)
            else:
                record.append(0)
        else:
        ...
        if record.count(1) == 0:
            result += 2
        elif record.count(1) == 2:
            result -= 2
        record = []
    return result
def ColumnTransitions(InputTetris, column):
    check = InputTetris.position()
    setup = {}
    record = []
    result = 0
    for i in range(len(check)):
        if check[i][0] not in setup:
            setup[check[i][0]] = [check[i][1]]
        else:
            setup[check[i][0]].append(check[i][1])
    for i in range(len(check)):
        if max(setup[check[i][0]]) + 1 < 19:
            if column[check[i][0]][max(setup[check[i][0]]) + 1] == '1':
                record.append(1)
            else:
                record.append(0)
        else:
...
def BoardBuriedHoles(column):
    ...
    for j in range(len(column)):
        ...
        record[j] = counter
    return record
def BoardBuriedHoles_Compare(InputTetris, column, board_holes_record):
    check = InputTetris.position()
    trial_column = []
    simple = {}
    for i in range(len(check)):
        if check[i][0] not in trial_column:
            trial_column.append(check[i][0])
            simple[check[i][0]] = [check[i][1]]
        else:
            simple[check[i][0]].append(check[i][1])
    for j in range(len(trial_column)):
        counter = 0    
        for k in range(min(simple[check[i][0]]), len(column[trial_column[j]])):
            if column[trial_column[j]][k] == '0':
                counter += 1
        previous = board_holes_record[trial_column[j]]
        counter -= previous
    return counter
def BoardWells(column):
    ...
    for j in range(len(column)):
        ...
        if j == 0:
            ...
            record[j] = count
    return record
def BoardWells_Compare(InputTetris, column, board_wells_record):
    result = 0
    check = InputTetris.position()
    trial_column = []
    simple = {}
    ...
    trial_column.sort()
    bottom_boundary = trial_column[0] - 1
    up_boundary = trial_column[-1] + 1
    count = 0
    for j in range(bottom_boundary, up_boundary + 1):
        if j < 0 or j >= len(column):
            continue
        if j == 0:
            empty = []
            for i in range(len(column[j])):
    ...
    for i in range(len(lists)):
        ...
        while lists[i] + 1 in lists:
            ...
            if i != len(lists) - 1:
                ...
                break
    return total
def evaluate(Allblocksign, ColumnSign, InputTetris, board_holes_record, board_wells_record):
    #print(ColumnSign)
    ...
    return LandingHeight(InputTetris, ColumnSign) * -45 + ErodedPieceCellsMetric(InputTetris, Allblocksign) * 34 - RowTransitions(InputTetris, Allblocksign) * 32 - ColumnTransitions(InputTetris, ColumnSign) * 93 - 79 * BoardBuriedHoles_Compare(InputTetris, ColumnSign, board_holes_record ) - 34 * BoardWells_Compare(InputTetris, ColumnSign, board_wells_record)
